...
class HandPosition:
    ...
    def center(self) -> tuple[float, float]:
        """Average position across all landmarks (normalised 0–1)."""
        if not self.landmarks:
            return (self.x, self.y)
        cx = sum(lx for lx, _ in self.landmarks) / len(self.landmarks)
        cy = sum(ly for _, ly in self.landmarks) / len(self.landmarks)
        return (cx, cy)
